...
export default function PetPage() {
  ...
  useEffect(() => {
    const loadCat = async () => {
      ...
      try {
        const activeCatColorId = user?.activeCatColorId;

        if (activeCatColorId == null) {
          setIsLoading(false);
          return;
        }
        ...
        const catLink = await drizzleDb
          ...
          .where(eq(schema.catColors.id, activeCatColorId))
          .get();
          if (!catLink) {
            setIsLoading(false);
            return;
        ...
        const activeCatHatId = user?.activeCatHatId;

        if (activeCatHatId == null) {
          setIsLoading(false);
          return;
        }
        ...
        const hatLink = await drizzleDb
          .select()
          .from(schema.catHats)
          .where(eq(schema.catHats.id, activeCatHatId))
          .get();
  ...
  const handleCatIconPress = async (
    spriteKey: keyof typeof catSprites,
    spriteImg: any,
    ...
    if (user.activeCatColorId == null || user.activeCatHatId == null) {
      setIsLoading(false);
      return;
    }
    await drizzleDb
  ...
  const handleHatIconPress = async (
      spriteKey: keyof typeof hatSprites,
      spriteImg: any,

  ...
  useEffect(() => {
    const loadApp = async () => {
      try {
        ...
        // Set demo user
        const demoUser = await db.select().from(schema.users).where(eq(schema.users.id, 1)).get();
        if (!demoUser) {
        ...
        if (demoUser.activeCatColorId == null) {
          console.log('No active cat color id found');
          return;
        }
        ...
        const catLink = await db
          ...
          .where(eq(schema.catColors.id, demoUser.activeCatColorId))
          .get();
        if (!catLink) {
          console.log('No cat link found');
          return;
        }

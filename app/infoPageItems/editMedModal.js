...
export default function EditMedModal({ visible, onClose, medications, onSave }) {
  ...
  useEffect(() => {
    const fetchMedicationData = async () => {
      if (visible && selectedMedId != null) {
        const med = await drizzleDb
          .select()
          .from(schema.userMedications)
          .where(eq(schema.userMedications.medicationId, selectedMedId))
          .get();
        
        const medNameList = await drizzleDb
          .select()
          .from(schema.medications)
          .where(eq(schema.medications.id, selectedMedId))
          .get();
        ...
        const sideEffectList = await drizzleDb
          ...
          .where(eq(schema.medicationSideEffects.medicationId, selectedMedId))
          .all();
        ...
        setFormData({
          ...
          generic: medNameList.generic,
          brand: medNameList.brand,
          dosageFormId: med.dosageFormId,
        ...
        <KeyboardAvoidingView behavior="padding" style={{ flex: 1 }}>
          <View style={styles.container}>
            <Text style={styles.header}>Edit Medication</Text>
              ...
              {formData && (
                <>
                <ScrollView style={{height: '60%', marginTop: 10}}>
                  <Text style={styles.label}>Generic Name</Text>
                  <TextInput style={styles.input} value={formData.generic} onChangeText={text => handleChange('generic', text)} />
                  <Text style={styles.label}>Brand Name</Text>
                  <TextInput style={styles.input} value={formData.brand}  onChangeText={text => handleChange('brand', text)}/>
